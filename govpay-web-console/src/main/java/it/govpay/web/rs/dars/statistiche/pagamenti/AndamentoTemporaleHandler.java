...
public class AndamentoTemporaleHandler extends StatisticaDarsHandler<AndamentoTemporale> implements IStatisticaDarsHandler<AndamentoTemporale>{
	...
	public AndamentoTemporaleHandler(Logger log, BaseDarsService darsService) { 
	...
	public PaginaGrafico getGrafico(UriInfo uriInfo, BasicBD bd) throws WebApplicationException, ConsoleException {
		...
		try{	
			...
			List<AndamentoTemporale> andamentoTemporale = pagamentiBD.getAndamentoTemporale(filter);
			...
			Grafico grafico = new Grafico(TipoGrafico.line);
			...
			grafico.getColori().addAll(Colori.getColoriAndamentoTemporale());
			grafico.getCategorie().add(Utils.getInstance(this.getLanguage()).getMessageFromResourceBundle(this.nomeServizio + ".numeroPagamenti.label"));

			if (andamentoTemporale != null && andamentoTemporale.size() > 0) {
				Serie<Long> serie1 = new Serie<Long>();
				for (AndamentoTemporale elemento : andamentoTemporale) {
					String dataElemento = _sdf.format(elemento.getData());
					long serie1Val = elemento.getNumero();

					grafico.getValoriX().add(dataElemento);

					serie1.getDati().add(serie1Val);
					serie1.getTooltip().add(Utils.getInstance(this.getLanguage()).getMessageWithParamsFromResourceBundle(this.nomeServizio + ".numeroPagamenti.tooltip", dataElemento, serie1Val));
				}
...
//				Utils.getInstance(this.getLanguage()).getMessageFromResourceBundle("statistiche.tipoIntervallo."+TipoIntervallo.ORARIO.name()+".value")));
	...
	@Override
	public String getTitolo(AndamentoTemporale entry,BasicBD bd) {
	...
	@Override
	public String getSottotitolo(AndamentoTemporale entry,BasicBD bd) {
		StringBuilder sb = new StringBuilder();
		return sb.toString();
	} 

	@Override
	public Map<String, Voce<String>> getVoci(AndamentoTemporale entry, BasicBD bd) throws ConsoleException {
...
//		}
	...
	@Override
	public InfoForm getInfoEsportazioneDettaglio(UriInfo uriInfo, BasicBD bd, AndamentoTemporale entry)	throws ConsoleException {	
		InfoForm infoEsportazione = null;
		return infoEsportazione;		
	}
