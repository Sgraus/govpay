...
public class OracleNativeQueries extends NativeQueries {
	...
	public String getRendicontazionePagamentoQuery() {
		return "SELECT " +
				...
				" p_importo_pagato,p_data_acquisizione,p_iur,p_data_pagamento,p_commissioni_psp,p_tipo_allegato,p_allegato,p_data_acquisizione_revoca,p_causale_revoca,p_dati_revoca,p_importo_revocato,p_esito_revoca,p_dati_esito_revoca,p_id,p_id_rpt,p_id_singolo_versamento,p_id_rr,p_id_incasso,p_iban_accredito,p_cod_dominio,p_iuv,p_stato_pagamento, " +
				" sv_cod_singolo_versamento_ente,sv_stato_singolo_versamento,sv_importo_singolo_versamento,sv_tipo_bollo,sv_hash_documento,sv_provincia_residenza,sv_tipo_contabilita,sv_codice_contabilita,sv_note,sv_id,sv_id_versamento,sv_id_tributo,sv_id_iban_accredito,tipo, inc_data_ora_incasso " +
				" FROM (SELECT " +
				...
				" p.importo_pagato as p_importo_pagato,p.data_acquisizione as p_data_acquisizione,p.iur as p_iur,p.data_pagamento as p_data_pagamento,p.commissioni_psp as p_commissioni_psp,p.tipo_allegato as p_tipo_allegato,p.allegato as p_allegato,p.data_acquisizione_revoca as p_data_acquisizione_revoca,p.causale_revoca as p_causale_revoca,p.dati_revoca as p_dati_revoca,p.importo_revocato as p_importo_revocato,p.esito_revoca as p_esito_revoca,p.dati_esito_revoca as p_dati_esito_revoca,p.id as p_id,p.id_rpt as p_id_rpt,p.id_singolo_versamento as p_id_singolo_versamento,p.id_rr as p_id_rr,p.id_incasso as p_id_incasso,p.iban_accredito as p_iban_accredito,p.cod_dominio as p_cod_dominio,p.iuv as p_iuv, p.stato as p_stato_pagamento, sv.cod_singolo_versamento_ente as sv_cod_singolo_versamento_ente,sv.stato_singolo_versamento as sv_stato_singolo_versamento,sv.importo_singolo_versamento as sv_importo_singolo_versamento,sv.tipo_bollo as sv_tipo_bollo,sv.hash_documento as sv_hash_documento,sv.provincia_residenza as sv_provincia_residenza,sv.tipo_contabilita as sv_tipo_contabilita,sv.codice_contabilita as sv_codice_contabilita,sv.note as sv_note,sv.id as sv_id,sv.id_versamento as sv_id_versamento,sv.id_tributo as sv_id_tributo,sv.id_iban_accredito as sv_id_iban_accredito " +
				"  " +
				...
				" UNION ALL " +
				" ( " +
				" SELECT 'STORNO' as tipo, r.iuv as r_iuv,r.iur as r_iur,r.importo_pagato as r_importo_pagato,r.esito as r_esito,r.data as r_data,r.stato as r_stato,r.anomalie as r_anomalie,r.id as r_id,r.id_fr as r_id_fr,r.id_pagamento as r_id_pagamento, p.importo_pagato as p_importo_pagato,p.data_acquisizione as p_data_acquisizione,p.iur as p_iur,p.data_pagamento as p_data_pagamento,p.commissioni_psp as p_commissioni_psp,p.tipo_allegato as p_tipo_allegato,p.allegato as p_allegato,p.data_acquisizione_revoca as p_data_acquisizione_revoca,p.causale_revoca as p_causale_revoca,p.dati_revoca as p_dati_revoca,p.importo_revocato as p_importo_revocato,p.esito_revoca as p_esito_revoca,p.dati_esito_revoca as p_dati_esito_revoca,p.id as p_id,p.id_rpt as p_id_rpt,p.id_singolo_versamento as p_id_singolo_versamento,p.id_rr as p_id_rr,p.id_incasso as p_id_incasso,p.iban_accredito as p_iban_accredito,p.cod_dominio as p_cod_dominio,p.iuv as p_iuv, p.stato as p_stato_pagamento, sv.cod_singolo_versamento_ente as sv_cod_singolo_versamento_ente,sv.stato_singolo_versamento as sv_stato_singolo_versamento,sv.importo_singolo_versamento as sv_importo_singolo_versamento,sv.tipo_bollo as sv_tipo_bollo,sv.hash_documento as sv_hash_documento,sv.provincia_residenza as sv_provincia_residenza,sv.tipo_contabilita as sv_tipo_contabilita,sv.codice_contabilita as sv_codice_contabilita,sv.note as sv_note,sv.id as sv_id,sv.id_versamento as sv_id_versamento,sv.id_tributo as sv_id_tributo,sv.id_iban_accredito as sv_id_iban_accredito " +
				...
				" ) ) s1 " +
				" left join fr on fr.id = s1.r_id_fr left join versamenti on versamenti.id = s1.sv_id_versamento left join incassi on incassi.id = s1.p_id_incasso $PLACEHOLDER_OUT$) $PLACEHOLDER_OFFSET_LIMIT$ order by fr_cod_flusso, p_data_pagamento";
	}
	...
	public String getPagamentoRendicontazioneQuery() {
		return "SELECT " +
				...
				" ) ) s1 " +
				" left join fr on fr.id = s1.r_id_fr left join versamenti on versamenti.id = s1.sv_id_versamento left join incassi on incassi.id = s1.p_id_incasso $PLACEHOLDER_OUT$) $PLACEHOLDER_OFFSET_LIMIT$ order by p_data_pagamento DESC, fr_cod_flusso";
	}
	...
	public String getStatisticheTransazioniPerEsitoQuery(TipoIntervallo tipoIntervallo, Date data, int limit, TransazioniFilter filtro) {
		...
		String sql = "select data, "
				...
				+ " from rpt right join SELECT ? + (INTERVAL '1' " + intervallo + " * level) AS data FROM dual CONNECT BY LEVEL <= ?) elencodate on trunc(rpt.data_msg_richiesta, ?) = trunc(elencodate.data, ?) ";
...
//				sql += " WHERE cod_psp = ? ";
	...
	public String getStatisticheTransazioniPerPspQuery(TipoIntervallo tipoIntervallo, Date data, TransazioniFilter filtro) {
		...
		String sql = "select psp.ragione_sociale, count(*) as totali"
				...
				+ " where trunc(data_msg_richiesta, ?) = trunc(?::timestamp, ?) ";
	...
	public String getStatistichePagamentiPerPspQuery(TipoIntervallo tipoIntervallo, Date data, PagamentiFilter filtro) {
		...
		String sql = "select psp.ragione_sociale, count(p.id) as totali"
				...
				+ " where trunc(p.data_pagamento, ?) = trunc(?::timestamp, ?) ";
	...
	@Override
	public Object[] getStatistichePagamentiAndamentoTemporaleValues(TipoIntervallo tipoIntervallo, Date data, int limit, PagamentiFilter filtro) {
		String date_trunc = "";
		switch (tipoIntervallo) {
		case MENSILE:
			date_trunc = "MM";
	...
	@Override
	public String getStatistichePagamentiAndamentoTemporaleQuery(TipoIntervallo tipoIntervallo, Date data, int limit,PagamentiFilter filtro) {
		String intervallo = "DAY";
		switch (tipoIntervallo) {
		case MENSILE:
			intervallo = "MONTH";
		...
		String placeHolderDate = " right join SELECT ? + (INTERVAL '1' " + intervallo + " * level) AS data FROM dual CONNECT BY LEVEL <= ?) elencodate on trunc(p.data_pagamento, ?) = trunc(elencodate.data, ?) ";
		boolean addSV = false;
