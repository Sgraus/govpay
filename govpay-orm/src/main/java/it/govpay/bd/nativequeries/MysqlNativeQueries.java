...
public class MysqlNativeQueries extends NativeQueries {
	...
	public String getRendicontazionePagamentoQuery() {
		return " SELECT " +
				...
				" ) ) as s1 " +
				" left join fr on fr.id = s1.r_id_fr left join versamenti on versamenti.id = s1.sv_id_versamento left join incassi on incassi.id = s1.p_id_incasso $PLACEHOLDER_OUT$ order by fr.cod_flusso, s1.p_data_pagamento $PLACEHOLDER_OFFSET_LIMIT$";
	}
	...
	public String getPagamentoRendicontazioneQuery() {
		return " SELECT " +
				...
				" ) ) as s1 " +
				" left join fr on fr.id = s1.r_id_fr left join versamenti on versamenti.id = s1.sv_id_versamento left join incassi on incassi.id = s1.p_id_incasso $PLACEHOLDER_OUT$ order by s1.p_data_pagamento DESC, fr.cod_flusso $PLACEHOLDER_OFFSET_LIMIT$";
	}
